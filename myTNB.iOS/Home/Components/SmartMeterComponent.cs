...
namespace myTNB.Dashboard.DashboardComponents
{
    public class SmartMeterComponent
    {
        ...
        internal void CreateComponent()
        {
            ...
            {
                Image = UIImage.FromBundle("Empty-Chart-Data"),
                ContentMode = UIViewContentMode.ScaleAspectFit
            };
            _viewSmartMeter.AddSubview(imgViewEmpty);

            UIView viewContent = new UIView(new CGRect(0, imgViewEmpty.Frame.GetMaxY() + 20
                , _viewSmartMeter.Frame.Width, 95));

            _lblSmartMeterTitle = new UILabel(new CGRect(0, 0, _viewSmartMeter.Frame.Width, 16))
            {
                TextAlignment = UITextAlignment.Center,
                Font = MyTNBFont.MuseoSans12_300,
                Text = "Component_WelcomeToSmartMeter".Translate(),
                TextColor = UIColor.White
            };
            viewContent.AddSubview(_lblSmartMeterTitle);

            _lblSmartMeterSubtitle = new UILabel(new CGRect(0, _lblSmartMeterTitle.Frame.GetMaxY() + 1
                , _viewSmartMeter.Frame.Width, 16))
            {
                TextAlignment = UITextAlignment.Center,
                Font = MyTNBFont.MuseoSans12_300,
                Text = "Component_SeeSmartMeter".Translate(),
                TextColor = UIColor.White
            };
