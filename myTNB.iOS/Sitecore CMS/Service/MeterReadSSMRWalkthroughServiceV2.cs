...
{
    public class MeterReadSSMRWalkthroughServiceV2
    {
        private string _os, _imgSize, _websiteURL, _language;
        internal MeterReadSSMRWalkthroughServiceV2(string os, string imageSize, string websiteUrl = null, string language = "en")
        {
            _os = os;
            _imgSize = imageSize;
        ...
        {
            string itemName = AppLaunchMasterCache.IsOCRDown ? Constants.Sitecore.ItemPath.MeterReadSSMRWalkthroughV2OCROff : Constants.Sitecore.ItemPath.MeterReadSSMRWalkthroughV2;
            ...
            try
            {
                for (int i = 0; i < itemsResponse.ResultCount; i++)
                {
                    ...
                    list.Add(new MeterReadSSMRModel
                    {
                        ...
                        Image = item.GetImageUrlFromItemWithSize(Constants.Sitecore.Fields.MeterReadSSMRWalkthrough.Image, _os, _imgSize, _websiteURL, _language).Replace(" ", "%20"),
                        ID = item.Id
                    });
                }
            }
