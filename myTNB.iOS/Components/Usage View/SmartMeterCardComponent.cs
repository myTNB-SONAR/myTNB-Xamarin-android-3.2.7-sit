...
namespace myTNB
{
    public class SmartMeterCardComponent : BaseComponent
    {
        ...
        private void CreateComponent()
        {
            ...
            {
                _toolTipView = new CustomUIView(new CGRect(BaseMarginWidth16
                    , GetScaledHeight(64F * 2) + GetScaledHeight(1F), width - (BaseMarginHeight16 * 2), GetScaledHeight(24F)))
                {
                    BackgroundColor = MyTNBColor.IceBlue
                };
                _toolTipView.Layer.CornerRadius = GetScaledHeight(12F);
                UIImageView toolTipIcon = new UIImageView(new CGRect(GetScaledWidth(4F)
                    , GetScaledHeight(4F), GetScaledWidth(16F), GetScaledHeight(16F)))
                {
                    Image = UIImage.FromBundle(Constants.IMG_InfoBlue)
                };
                _toolTipLabel = new UILabel(new CGRect(toolTipIcon.Frame.GetMaxX() + GetScaledWidth(8F)
                    , GetScaledHeight(4F), _toolTipView.Frame.Width * .80F, GetScaledHeight(16F)))
                {
                ...
                if (model.Value.IsValid())
                {
                    ...
                    else
                    {
                        ...
                        if (isCurrentUsage)
                        {
                            ...
                            valueUnitStr = model.ValueUnit.IsValid() ? model.ValueUnit : Constants.UnitEnergy;
                            amountStr = string.Format("{0:n0}", currentUsage);
                        }
                        else
                        {
                            valueUnitStr = model.ValueUnit.IsValid() ? model.ValueUnit : TNBGlobal.UNIT_CURRENCY;
        ...
        public virtual CustomUIView GetShimmerUI()
        {
            ...
            UIView line = new UIView(new CGRect(BaseMarginWidth16, GetScaledHeight(64F)
                , parentView.Frame.Width - (BaseMarginWidth16 * 2), GetScaledHeight(1F)))
            {
                BackgroundColor = MyTNBColor.VeryLightPinkThree
            };
            ...
            UIView tooltipView = new CustomUIView(new CGRect(BaseMarginWidth16
                , GetScaledHeight(64F * 2) + GetScaledHeight(1F), parentView.Frame.Width - (BaseMarginWidth16 * 2), GetScaledHeight(24F)))
            {
        ...
        private UIView ItemDetailShimmerView(UIView parentView, nfloat yPos)
        {
            ...
            UIView titleView = new UIView(new CGRect(iconView.Frame.GetMaxX() + GetScaledWidth(12)
                , BaseMarginHeight16, width * .30F, GetScaledHeight(16F) * 0.8F))
            {
                BackgroundColor = MyTNBColor.PaleGrey
            };
            titleView.Layer.CornerRadius = GetScaledHeight(2f);
            UIView dateRangeView = new UIView(new CGRect(iconView.Frame.GetMaxX() + GetScaledWidth(12)
                , titleView.Frame.GetMaxY() + GetScaledHeight(5f), width * .20F, GetScaledHeight(16F) * 0.7F))
            {
                BackgroundColor = MyTNBColor.PaleGrey
            };
            dateRangeView.Layer.CornerRadius = GetScaledHeight(2f);
            UIView amountView = new UIView(new CGRect(width * .75F - BaseMarginWidth16, GetScaledHeight(22F)
                , width * .25F, GetScaledHeight(20F) * 0.8F))
            {
                BackgroundColor = MyTNBColor.PaleGrey
            };
