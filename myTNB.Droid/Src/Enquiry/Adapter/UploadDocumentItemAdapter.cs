...
namespace myTNB.AndroidApp.Src.Enquiry.Adapter
{
    public class UploadDocumentItemAdapter : BaseRecyclerAdapter<AttachedImage>
    {
        ...
        public UploadDocumentItemAdapter(bool notify) : base(notify) { }

        public UploadDocumentItemAdapter(List<AttachedImage> itemList) : base(itemList) { }

        public UploadDocumentItemAdapter(List<AttachedImage> itemList, bool notify) : base(itemList, notify) { }
        ...
        {
            SelectClickEvent?.Invoke(this, position);
        }
        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
        {
            ...
            if (holder is UploadDocumentItemImageViewHolder)
            {
                //actual image view
                var viewHolder = holder as UploadDocumentItemImageViewHolder;
                ...
                if (isPDF == true)
                {
                    Picasso.With(viewHolder.ItemView.Context)
                     ...
                     .Fit()
                     .Into(viewHolder.imageView, delegate
                     {
            ...
            {
                //empty view
                var viewHolder = holder as UploadDocumentItemEmptyViewHolder;
                if (image.IsLoading)
                {
        ...
        {
            RemoveClickEvent?.Invoke(this, position);
        }

        private void AddClick(int position)
        {
            AddClickEvent?.Invoke(this, position);
        }
        ...
        public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
        {
            ...
            {
                return new UploadDocumentItemImageViewHolder(LayoutInflater.From(parent.Context).Inflate(Resource.Layout.UploadDocumentItemAdapterImageView, parent, false), RemoveClick);
            }
            else
            {
                return new UploadDocumentItemEmptyViewHolder(LayoutInflater.From(parent.Context).Inflate(Resource.Layout.UploadDocumentItemAdapterEmptyView, parent, false), AddClick);
            }
        }

        class UploadDocumentItemImageViewHolder : BaseRecyclerViewHolder
        {
            ...
            public UploadDocumentItemImageViewHolder(View itemView, Action<int> listener) : base(itemView)
            {
                btnClose.Click += (sender, e) => listener(base.LayoutPosition);
            }
        }

        class UploadDocumentItemEmptyViewHolder : BaseRecyclerViewHolder
        {
            ...
            public UploadDocumentItemEmptyViewHolder(View itemView, Action<int> listener) : base(itemView)
            {
